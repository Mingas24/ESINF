package GrafoMatrizDeAdjacencias;
...
public class AdjacencyMatrixGraph<V, E> implements BasicGraph<V, E>, Cloneable {
    ...
    public String toString() {
        ...
        for (int i = 0; i < numVertices; i++) {
            for (int j = 0; j < numVertices; j++) {
                if (edgeMatrix[i][j] != null) {
                    sb.append("From " + vertices.get(i)+ i + " to " + vertices.get(j) + j + "-> " + edgeMatrix[i][j] + "\n");
                }
            }
        }
     ...
     *
     * the other graph to test for equality
